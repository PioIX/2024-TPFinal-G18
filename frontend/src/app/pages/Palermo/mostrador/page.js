...
import styles from '@/app/pages/Palermo/mostrador/mostradorpalermo.module.css'; // Cambié styles por Styles para que coincida con la importación
...
export default function Game() {
  ...
  if (!soundRef.current) {
    soundRef.current = new Audio('/sound/Curiosidad.mp3'); // Asegúrate de que la ruta sea correcta
  }
  ...
  const handlePlayMusic = () => {
    ...
    async function obtenerClientesMezcladosPorNivel() {
        try {
            const response = await fetch("http://localhost:4000/clientesPorEscenario?idEscenario=4", {
                method: "GET",
                headers: {
                ...
                setTimeout(() => {
                    ...
                    setTimeout(() => {
                        if (score >= 2900) {
                            ...
                            router.push('/pages/Puerto_Madero/mostrador');
                        } else {
    ...
    return (
        <div className={styles.imgPalermo}>
            <Score score={score} />
