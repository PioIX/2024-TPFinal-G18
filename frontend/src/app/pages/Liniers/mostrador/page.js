...
import styles from '@/app/pages/Liniers/mostrador/mostradorliniers.module.css'; // Cambié styles por Styles para que coincida con la importación
...
export default function Game() {
  ...
  if (!soundRef.current) {
    soundRef.current = new Audio('/sound/Cariñito.mp3'); // Asegúrate de que la ruta sea correcta
  }
  ...
  useEffect(() => {
    ...
    async function obtenerClientesMezcladosPorNivel() {
        try {
            const response = await fetch("http://localhost:4000/clientesPorEscenario?idEscenario=2", {
                method: "GET",
                headers: {
                ...
                setTimeout(() => {
                    ...
                    setTimeout(() => {
                        if (score >= 1900) {
                            ...
                            router.push('/pages/Floresta/mostrador');
                        } else {
